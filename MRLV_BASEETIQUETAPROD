...
WHERE EXISTS     (SELECT 1 FROM MRL_ENCARTE X INNER JOIN MRL_ENCARTEPRODUTO XI      ON XI.SEQENCARTE = X.SEQENCARTE
                           ...
                           WHERE 1=1
                             -- Apenas os que possuem preco meu nagumo
                             AND NVL(PP.PRECOCARTAO,0) > 0
                             -- Retornando as saidas do dia anterior
                             AND X.DTAFIM = TRUNC(SYSDATE) - 1
                             AND XE.NROEMPRESA = A.NROEMPRESA
                             -- Retorna produtos da familia do produto que esta no encarte
                             AND(SF.SEQPRODUTO = A.SEQPRODUTO
                             -- Retorna produtos que estao na similaridade do produto que esta no encarte
...
-- Por fim, select normal da aplicacao
...
create or replace view mrlv_baseetiquetaprod_bkp as
select a.seqproduto,
       a.nroempresa,
       a.nrosegmento,
       b.desccompleta,
