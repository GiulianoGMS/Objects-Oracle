CREATE OR REPLACE PROCEDURE NAGP_EXT_TTMS_NOTAS_ENTSAI (vsDtaInicial DATE, vsDtaFinal DATE, psNroEmpresa NUMBER) IS
...
BEGIN
    ...
    SELECT 'CNPJ_EMITENTE;CNPJ_DESTINATARIO;COD_ITEM;DESC_ITEM;COD_FAMILIA;DESC_FAMILIA;NCM;CFOP;CST_ICMS;CST_IPI;CST_PIS;CST_COFINS;VALOR_PROD;VALOR_ICMS;VALOR_ICMSST;VALOR_IPI;VALOR_PIS;VALOR_COFINS;VLR_BASE_PIS;VLR_BASE_COFINS'
      INTO v_Cabecalho
      FROM DUAL A;
    ...
    -- Executa a query e escreve os resultados
      ...
      FOR bs IN (SELECT X.CNPJ_EMITENTE, /*X.CHAVE,*/ LPAD(G.NROCGCCPF,12,0)||LPAD(G.DIGCGCCPF,2,0) CNPJ_DESTINATARIO, X.CODIGO_ITEM COD_ITEM,
       ...
       SUM(X.VLR_TOTAL_PRODUTOS) VLR_TOTAL_PRODUTOS, SUM(X.VLR_TOTAL_ICMS) VLRICMS, SUM(X.VLRICMSST) VLRICMSST, SUM(X.VLRIPI) VLRIPI, SUM(X.VLR_TOTAL_PIS) VLRPIS, SUM(X.VLR_TOTAL_COFINS) VLRCOFINS,
       --FVALORTOTALNF(X.ID, 'N') VALOR_TOTAL_NF, 
       SUM(X.VLR_BASE_CALC_PIS) VLR_BASE_CALC_PIS, SUM(X.VLR_BASE_CALC_COFINS) VLR_BASE_CALC_COFINS
   ...
   AND X.NROEMPRESA IN (SELECT SEQPESSOA
                         ...
                         WHERE LPAD(NROCGCCPF, 12, 0) || LPAD(DIGCGCCPF, 2, 0) IN
                               (07705530000184,
                                ...
                                GROUP BY X.CNPJ_EMITENTE, /*X.CHAVE,*/ LPAD(G.NROCGCCPF,12,0)||LPAD(G.DIGCGCCPF,2,0), X.CODIGO_ITEM,
       X.DESC_ITEM, P.SEQFAMILIA, FAMILIA, X.NCM, X.CFOP, X.ORIGEM_ICMS||X.CST_ICMS, X.SITUACAONFIPI, X.CST_PIS, X.CST_COFINS
       
                                
UNION ALL

SELECT X.CNPJ_EMITENTE, /*X.CHAVE,*/ LPAD(G.NROCGCCPF,12,0)||LPAD(G.DIGCGCCPF,2,0) CNPJ_DESTINATARIO, X.CODIGO_ITEM,
       X.DESC_ITEM, P.SEQFAMILIA COD_FAMILIA, FAMILIA DESC_FAMILIA, X.NCM, X.CFOP, X.ORIGEM_ICMS||X.CST_ICMS CST_ICMS, X.SITUACAONFIPI CST_IPI, X.CST_PIS, X.CST_COFINS,
       SUM(X.VLR_TOTAL_PRODUTOS), SUM(X.VLR_TOTAL_ICMS) VLRICMS, SUM(X.VLRICMSST), SUM(X.VLRIPI), SUM(X.VLR_TOTAL_PIS) VLRPIS, SUM(X.VLR_TOTAL_COFINS) VLRCOFINS,
       --FVALORTOTALNF(X.ID, 'N') VALOR_TOTAL_NF, 
       SUM(X.VLR_BASE_CALC_PIS) VLR_BASE_CALC_PIS, SUM(X.VLR_BASE_CALC_COFINS) VLR_BASE_CALC_COFINS
   ...
   AND X.NROEMPRESA IN (SELECT SEQPESSOA
                         ...
                         WHERE LPAD(NROCGCCPF, 12, 0) || LPAD(DIGCGCCPF, 2, 0) IN
                               (07705530000184,
                                ...
                                15379973000250,
                                03376054000144,
                                13941798000118)) GROUP BY X.CNPJ_EMITENTE, /*X.CHAVE,*/ LPAD(G.NROCGCCPF,12,0)||LPAD(G.DIGCGCCPF,2,0), X.CODIGO_ITEM,
       X.DESC_ITEM, P.SEQFAMILIA, FAMILIA, X.NCM, X.CFOP, X.ORIGEM_ICMS||X.CST_ICMS, X.SITUACAONFIPI, X.CST_PIS, X.CST_COFINS
      ...
      LOOP
         ...
         v_line :=  bs.CNPJ_EMITENTE||';'/*||bs.CHAVE||';'*/||bs.CNPJ_DESTINATARIO||';'||bs.COD_ITEM||';'||bs.DESC_ITEM||';'||bs.COD_FAMILIA||';'||bs.DESC_FAMILIA||';'||bs.NCM||';'||bs.CFOP||';'||bs.CST_ICMS||';'||bs.CST_IPI||';'||bs.CST_PIS||';'||bs.CST_COFINS||';'||bs.VLR_TOTAL_PRODUTOS||';'||bs.VLRICMS||';'||bs.VLRICMSST||';'||bs.VLRIPI||';'||bs.VLRPIS||';'||bs.VLRCOFINS||';'||bs.VLR_BASE_CALC_PIS||';'||bs.VLR_BASE_CALC_COFINS;
