CREATE OR REPLACE PROCEDURE NAGP_CRM_EXT_PESSOA IS
...
BEGIN
    ...
    v_file := UTL_FILE.fopen('PLUSOFT', 'Ext_CRM_Pessoa_'||v_Periodo||'.csv', 'w', 32767);

    -- Pega o nome das colunas para inserir no cabecalho pq tenho preguica
    SELECT LISTAGG(COLUMN_NAME,';') WITHIN GROUP (ORDER BY COLUMN_ID)
      INTO v_Cabecalho
      FROM ALL_TAB_COLUMNS A
     WHERE A.table_name = 'NAGV_CRM_PESSOA_EXT'
    ...
    -- Executa a query e escreve os resultados
      ...
      FOR vda IN ( SELECT IDUNICO,
                          ...
                          ESTADO,
                          CANAL_CADASTRO                               
                    FROM NAGV_CRM_PESSOA_EXT X
                   WHERE 1=1 
                     ) 

      LOOP

      v_line := vda.IDUNICO||';'||vda.IDPESSOA||';'||vda.TXTNOMECOMPLETO||';'||vda.TXTESTADOCIVIL||';'||vda.TXTSEXO||';'||
                vda.DATNASCIMENTO||';'||vda.DTAPRIMCOMPRA||';'||vda.DTAULTCOMPRA||';'||vda.DATATUALIZACAOCADASTRO||';'||vda.DATCADASTRO||';'||vda.NUMCEL1||';'||
                vda.NUMCEL2||';'||vda.NUMTELFIXO||';'||vda.NUMTELCOMERCIAL||';'||vda.TXTEMAIL||';'||vda.TXTCEP||';'||vda.TXTENDERECO||';'||vda.TXTBAIRRO||';'||
                vda.TXTCIDADE||';'||vda.ESTADO||';'||vda.CANAL_CADASTRO;
