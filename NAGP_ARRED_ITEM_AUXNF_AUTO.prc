CREATE OR REPLACE PROCEDURE NAGP_ARRED_ITEM_AUXNF_AUTO ( vnSeqAuxNF NUMBER ) --, psNroEmpresa NUMBER, pdSeqPessoa NUMBER)
...
-- Pega o valor total do XML para comparar se vai somar ou subtrair centavos do lancamento
   ...
   SELECT M000_VL_NF
     INTO vlTotalXML
     FROM TMP_M000_NF F WHERE F.M000_NR_CHAVE_ACESSO = psChave;
