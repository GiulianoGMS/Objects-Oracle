CREATE OR REPLACE PROCEDURE NAGP_ARRED_ITEM_AUXNF_AUTO (pdChaveNF NUMBER, psNroEmpresa NUMBER, pdSeqPessoa NUMBER)
...
-- Pega SeqAuxNotaFiscal para selects posteriores e tipo do cgo para seguir com tratamento
  ...
  SELECT SEQAUXNOTAFISCAL, C.TIPDOCFISCAL
    INTO psSeqAuxNF, psTipoCGO
    FROM MLF_AUXNOTAFISCAL N INNER JOIN MAX_CODGERALOPER C ON C.CODGERALOPER = N.CODGERALOPER
   WHERE N.NFECHAVEACESSO = pdChaveNF AND NROEMPRESA = psNroEmpresa AND SEQPESSOA = pdSeqPessoa;
...
-- Pega o valor total do XML para comparar se vai somar ou subtrair centavos do lancamento
   ...
   SELECT M000_VL_NF
     INTO vlTotalXML
     FROM TMP_M000_NF F WHERE F.M000_NR_CHAVE_ACESSO = pdChaveNF;
