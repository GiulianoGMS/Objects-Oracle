...
BEGIN
  ...
  FOR t IN (SELECT A.CODESPECIE, X.NROACORDO, NVL(P.DTAVENCIMENTO, X.DTAVENCIMENTO) DTAVENCIMENTO, NVL(P.NROPARCELA,1) NROPARCELA,
                   X.SEQFORNECEDOR, NVL(P.VALORPARCELA, X.VLREXPFINANCEIRO) VLREXPFINANCEIRO,
                   X.DTAEMISSAO
             ...
             WHERE X.SITUACAOACORDO = 'F' AND X.NROACORDO = psNroAcordo AND X.NROEMPRESA = psNroEmpresa
  ...
  vnSeqLancto      := consinco.PKG_FINANCEIRO.ffip_buscaseq();
	vnSEQTITULO      := consinco.PKG_FINANCEIRO.ffip_buscaseq();
  vnSeqTitOperacao := consinco.PKG_FINANCEIRO.ffip_buscaseq();
