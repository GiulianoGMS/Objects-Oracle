...
SELECT /*+OPTIMIZER_FEATURES_ENABLE('19.1.0')*/
      ...
      -- Aliquota != 0 e CST IPI Diferente de 00
       ...
       CASE WHEN EXISTS (
        ...
        /*CASE WHEN EXISTS(
          SELECT DISTINCT F.SEQFAMILIA, F.ALIQUOTAIPI, DP.ALIQUOTAIPI
            FROM MAP_FAMFORNEC FC INNER JOIN MAP_FAMILIA F ON F.SEQFAMILIA = FC.SEQFAMILIA
                                  INNER JOIN NAGT_DEPARA_TICKET464111 DP ON DP.CODNBMSH = F.CODNBMSH
           WHERE FC.SEQFORNECEDOR IN (502,503)
             AND EXISTS (SELECT 1 FROM MAP_FAMFORNEC EX INNER JOIN GE_PESSOA G ON G.SEQPESSOA = EX.SEQFORNECEDOR
                          WHERE EX.SEQFAMILIA = FC.SEQFAMILIA
                            AND UF = 'EX')
             AND NVL(F.ALIQUOTAIPI,0) != DP.ALIQUOTAIPI
             AND F.SEQFAMILIA = MP.SEQFAMILIA)
        THEN '(EX) Aliquota da IPI na familia é diferente da Alíquota de IPI a Regra - C5: '
             ||(SELECT FF.ALIQUOTAIPI||' - Regra: '||XX.ALIQUOTAIPI
                  FROM MAP_FAMILIA FF INNER JOIN (SELECT DISTINCT * FROM NAGT_DEPARA_TICKET464111) XX ON XX.CODNBMSH = FF.CODNBMSH
                 WHERE FF.SEQFAMILIA = MP.SEQFAMILIA)                                     END*/ NULL INC13
