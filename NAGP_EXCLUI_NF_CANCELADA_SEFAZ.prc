CREATE OR REPLACE PROCEDURE CONSINCO.NAGP_EXCLUI_NF_CANCELADA_SEFAZ AS
  ...
  BEGIN
        FOR t IN (SELECT /*+OPTIMIZER_FEATURES_ENABLE('11.2.0.4')*/ DISTINCT A.CHAVEACESSO, A.SEQNOTAFISCAL, INSTR(A.SEQNOTAFISCAL,1) SEQUENCIAL,
                 (SELECT SEQAUXNOTAFISCAL FROM CONSINCO.MLF_AUXNOTAFISCAL AUX WHERE AUX.NFECHAVEACESSO = A.CHAVEACESSO) SeqAuxNotaFiscal
                   ...
                   WHERE 1=1
                     AND (B.INDCANCELREJEICAO = 'C' OR NI.TIPOINCONSIST = 'N' AND NI.CODINCONSIST = 14)
