CREATE OR REPLACE PROCEDURE CONSINCO.NAGP_EXCLUI_NF_CANCELADA_SEFAZ AS
  ...
  BEGIN
        FOR t IN (SELECT DISTINCT A.CHAVEACESSO, SEQNOTAFISCAL, INSTR(SEQNOTAFISCAL,1) SEQUENCIAL,
                 (SELECT SEQAUXNOTAFISCAL FROM CONSINCO.MLF_AUXNOTAFISCAL AUX WHERE AUX.NFECHAVEACESSO = A.CHAVEACESSO) SeqAuxNotaFiscal
                    FROM CONSINCO.MRLV_NFEIMPORTACAO A INNER JOIN CONSINCO.MRL_NFEIMPORT_CANCREJ B ON A.CHAVEACESSO = B.NFECHAVEACESSO
