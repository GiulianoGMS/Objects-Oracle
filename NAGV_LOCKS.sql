...
SELECT INITCAP(NVL(GV$SESSION.USERNAME, '{Oracle}')) SO, GV$SESSION.SID, GV$SESSION.SERIAL#,
 ...
 WHERE GV$SESSION.USERNAME IS NOT NULL
  ...
  AND EXISTS (SELECT X.BLOCKING_SESSION FROM GV$SESSION X WHERE DECODE(GV$SESSION.BLOCKING_SESSION, NULL, NULL, 'SID ' || GV$SESSION.BLOCKING_SESSION)  IS NOT NULL AND X.BLOCKING_SESSION = GV$SESSION.SID)
