CREATE OR REPLACE VIEW MRLV_ETIQ_VALIDADE AS
SELECT /*+OPTIMIZER_FEATURES_ENABLE('11.2.0.4')*/
       ...
       ||
      --'^FO410,0^GB4,400,4^FS'                                                         || CHR(13) || CHR(10) || -- Risco Vertical (divisor)
      ...
      '^FO13,110^A0N,55,55^FD'||LPAD(TRUNC(B.VLRPRECOPROMOC), 4, ' ' ) || ',' || LPAD((B.VLRPRECOPROMOC - TRUNC(B.VLRPRECOPROMOC)) * 100, 2, 0)||'^FS' || CHR(13) || CHR(10) || -- PROMOC
      '^FO115,200^A0N,18,18^FD*PRODUTO PROXIMO DA VALIDADE^FS'                        || CHR(13) || CHR(10) ||
      '^FO115,228^A0N,18,18^FD*SEM TROCA   Cod Prod: '||A.SEQPRODUTO||'^FS'           || CHR(13) || CHR(10) ||
      ...
      '^FO427,110^A0N,55,55^FD'||LPAD(TRUNC(B.VLRPRECOPROMOC), 4, ' ' ) || ',' || LPAD((B.VLRPRECOPROMOC - TRUNC(B.VLRPRECOPROMOC)) * 100, 2, 0)||'^FS' || CHR(13) || CHR(10) || -- PROMOC
      '^FO530,200^A0N,18,18^FD*PRODUTO PROXIMO DA VALIDADE^FS'                        || CHR(13) || CHR(10) ||
      '^FO530,228^A0N,18,18^FD*SEM TROCA   Cod Prod: '||A.SEQPRODUTO||'^FS'           || CHR(13) || CHR(10) ||
...
FROM  CONSINCO.MRLX_BASEETIQUETAPROD A INNER JOIN consinco.MRLV_PROMOCAOESPECIAL B ON A.SEQPRODUTO = B.SEQPRODUTO AND A.NROEMPRESA = B.NROEMPRESA
