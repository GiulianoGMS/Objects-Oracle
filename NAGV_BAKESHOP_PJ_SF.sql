...
SELECT /*+OPTIMIZER_FEATURES_ENABLE('19.1.0')*/
       ...
       NVL(N.BASEFCPST,NVL(ESP_FCALCVALORICMSULTENTRADA(Z.SEQPRODUTO, S.NROEMPRESA,'BF', TRUNC(SYSDATE),810), 0))           vBCFCPRet,
       NVL(N.PERALIQFCPST,NVL(ESP_FCALCVALORICMSULTENTRADA(Z.SEQPRODUTO, S.NROEMPRESA,'AF', TRUNC(SYSDATE),810), 0))        pFCPSTRet,
       NVL(N.VLRFCPST, NVL(ESP_FCALCVALORICMSULTENTRADA(Z.SEQPRODUTO, S.NROEMPRESA,'VF', TRUNC(SYSDATE),810), 0))           vFCPSTRet,
  ...
  FROM MAP_PRODCODIGO Z INNER JOIN MAP_PRODUTO P       ON P.SEQPRODUTO = Z.SEQPRODUTO
                        ...
                        INNER JOIN MRL_PRODEMPSEG S    ON S.SEQPRODUTO = Z.SEQPRODUTO AND NROSEGMENTO = 2 AND S.QTDEMBALAGEM = 1 AND S.NROEMPRESA = 57
