...
UNION -- Comeca Select dos Combos Promocionais
...
FROM MRL_PRODEMPSEG A INNER JOIN MAP_PRODUTO B ON A.SEQPRODUTO = B.SEQPRODUTO
                      ...
                      INNER JOIN MAD_FAMSEGMENTO F    ON F.SEQFAMILIA = B.SEQFAMILIA AND F.NROSEGMENTO = A.NROSEGMENTO
                      -- Join nas Ativaveis
                      INNER JOIN NAGV_BASE_PROMOC_PDV_ETQ PPDV ON PPDV.SEQPRODUTO = A.SEQPRODUTO AND PPDV.NROEMPRESA = A.NROEMPRESA

WHERE (TRUNC(SYSDATE) = PPDV.DTAINICIO OR TRUNC(SYSDATE) -1 = PPDV.DTAFIM)
...
UNION -- Comeca Select das Regras de Incentivo
...
FROM MRL_PRODEMPSEG A INNER JOIN MAP_PRODUTO B ON A.SEQPRODUTO = B.SEQPRODUTO
                      ...
                      INNER JOIN MAX_EMPRESA EMP      ON EMP.NROEMPRESA = A.NROEMPRESA
                      INNER JOIN MAD_FAMSEGMENTO F    ON F.SEQFAMILIA = B.SEQFAMILIA AND F.NROSEGMENTO = A.NROSEGMENTO
                      -- Join na Regra de Incentivo
                      INNER JOIN NAGV_PROMOC_ATIVAS_REGRAPDV_ETQ RPDV ON RPDV.SEQPRODUTO = A.SEQPRODUTO AND RPDV.NROEMPRESA = A.NROEMPRESA

WHERE (TRUNC(SYSDATE) = RPDV.DTAINICIO OR TRUNC(SYSDATE) -1 = RPDV.DTAFIM)
  AND D.QTDEMBALAGEM = DECODE(NVL(FC5MAXPARAMETRO('EMISSAO_ETIQUETA',
                                                       A.NROEMPRESA,
                                                       'EMITE_ETIQ_PRECO_DIF'),
                                   'N'), 'N', F.PADRAOEMBVENDA, 'M',
      ...
      -- Corta ja emitidos (De acordo com data de impressao)
