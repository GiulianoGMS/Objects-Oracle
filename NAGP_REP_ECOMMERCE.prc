CREATE OR REPLACE PROCEDURE NAGP_REP_ECOMMERCE (psCodPromocao NUMBER DEFAULT NULL,
                                               psData         DATE DEFAULT NULL) AS
...
BEGIN
  ...
  FOR capa IN (SELECT S_SEQPROMOCPDV.NEXTVAL SEQPROMOCPDV, CODPROMOCAO, DTINICIO, DTFIM,
                 ...
                 FROM (SELECT DISTINCT CODPROMOCAO, T.DTINICIO, T.DTFIM
                        ...
                        WHERE 1=1 AND T.CODPROMOCAO = NVL(psCodPromocao, T.CODPROMOCAO)
                          ...
                          AND TRUNC(T.DTINICIO) = NVL(psData,TRUNC(SYSDATE))
  ...
  FOR item_loja IN (SELECT DISTINCT CODPROMOCAO, CODLOJA, PRECOPPROMOCIONAL, item_Base.SEQITEMPROMOC,
                           PRECOVALIDNORMAL PRECO_NORMAL, --NVL(NULLIF(S.PRECOVALIDPROMOC,0), S.PRECOVALIDNORMAL) PRECO_NORMAL, 
                           PRECOVALIDNORMAL - PRECOPPROMOCIONAL VLRDESCONTO, --NVL(NULLIF(S.PRECOVALIDPROMOC,0), S.PRECOVALIDNORMAL) - PRECOPPROMOCIONAL VLRDESCONTO,
                           ...
                           ROUND(((PRECOVALIDNORMAL - PRECOPPROMOCIONAL) / PRECOVALIDNORMAL) * 100 ,2) PERCDESCONTO,
                     ...
                     WHERE T.CODPROMOCAO = capa.CODPROMOCAO
                       ...
                       AND T.PROMOCAOLIVRE = 0
                       AND EXISTS (SELECT 1 FROM MAX_EMPRESA A WHERE A.NROEMPRESA = CODLOJA)
                       AND PRECOVALIDNORMAL > 0 --AND NVL(NULLIF(S.PRECOVALIDPROMOC,0), S.PRECOVALIDNORMAL) > 0
                       -- Apenas se vlr promocao é menor que o preco normal
                       AND PRECOPPROMOCIONAL < PRECOVALIDNORMAL)
                       --AND NVL(NULLIF(S.PRECOVALIDPROMOC,0), S.PRECOVALIDNORMAL) - PRECOPPROMOCIONAL > 0)
