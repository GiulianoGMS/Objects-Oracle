CREATE OR REPLACE VIEW CONSINCO.MRLV_ETIQ_NAGUMO_VAREJO_VM3 AS
...
--======== INICIO OFERTAS ATIVAVEIS (MEU NAGUMO) - 27/08/2024 - Giuliano ========--
        ...
        WHEN AT.PRECOPROMOCIONAL < TRUNC((A.PRECOVALIDNORMAL * A.PADRAOEMBVENDA),2) AND A.NROEMPRESA NOT IN (31,41,54)
      THEN                                   CHR(13) || CHR(10) ||
        ...
        '^FO690,85^A0N,090,40^FD  ^FS'|| CHR(13) || CHR(10) ||
        '^FO050,180^BY2.4^BEN,25,Y,N^FD'||A.CODACESSOPADRAO||'^FS'|| CHR(13) || CHR(10) ||
        '^FO50,95^A0N,25,25^FDR$^FS'|| CHR(13) || CHR(10) ||
        ...
        ELSE
        '^FO40,70^A0N,72,72FD'||LPAD(TRUNC(J.PRECOVALIDNORMAL), 4, ' ' ) || ',' || LPAD((J.PRECOVALIDNORMAL - TRUNC(J.PRECOVALIDNORMAL)) * 100, 2, '0')|| '^FS' END)|| CHR(13) || CHR(10) || -- PRECO NORMAL
...
--======== INICIO OFERTAS EXCLUSIVAS - APENAS MEU NAGUMO SEM OFERTA C5 - 02/09/2024 - Giuliano ========--
        ...
        WHEN NVL(J.PRECOVALIDPROMOC,0) = 0 AND RPM.PRECOPPROMOCIONAL < TRUNC((A.PRECOVALIDNORMAL * A.PADRAOEMBVENDA),2) AND A.NROEMPRESA NOT IN (31,41,54)
      THEN                                   CHR(13) || CHR(10) ||
        '^FO50,20^A0N,40,40^FD'||SUBSTR(A.DESCCOMPLETA,0,37) ||' '||CASE WHEN J.QTDEMBALAGEM > 1 THEN J.EMBALAGEM ELSE NULL END||'^FS' || CHR(13) || CHR(10) || -- DESC
        '^FO50,251^A0N,20,20^FD'||TO_CHAR(SYSDATE, 'DD/MM/YY HH24:MI')||'   Produto: '||A.SEQPRODUTO||'   Val. Prom.: '||
