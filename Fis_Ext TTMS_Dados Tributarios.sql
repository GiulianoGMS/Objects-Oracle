SELECT DECODE(T.TIPTRIBUTACAO, 'EI', 'Entada Industria', 'ED', 'Entrada Distribuidor', 'EM', 'Entrada Microempresa', 'SC', 'Saida Contribuinte', 'SN', 'Saida Não Contrinuinte') TIPO_TRIBUTACAO,
  ...
  FROM MAP_PRODUTO P INNER JOIN MAP_FAMILIA F ON F.SEQFAMILIA = P.SEQFAMILIA
                     ...
                     INNER JOIN MAP_FINALIDADEFAMILIA FF ON FF.FINALIDADEFAMILIA = FD.FINALIDADEFAMILIA
                      LEFT JOIN NAGT_PDV_CENARIOPRODUTO CC ON CC.SEQPRODUTO = P.SEQPRODUTO AND CC.CIDADE = 'SAO PAULO'
                     INNER JOIN MAP_TRIBUTACAOUF T ON T.NROTRIBUTACAO = FD.NROTRIBUTACAO

WHERE 1=1 --P.SEQPRODUTO = 262581
