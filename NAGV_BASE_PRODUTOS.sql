-- No DW
CREATE OR REPLACE VIEW NAGV_BASE_PRODUTOS AS
SELECT 
       X.NROEMPRESA                                     ,
       X.SEQPRODUTO                                     PRODUCT_CODE,
       FBUSCAEANPRODUTONAG@LINK_C5(X.SEQPRODUTO)                CODIGO_BARRA,
       P.PRODUTO                                   PROD_DESCRIPTION,
       (E.ESTQLOJA + E.ESTQDEPOSITO - E.QTDRESERVADAVDA -
            E.QTDRESERVADARECEB - E.QTDRESERVADAFIXA +
          FC5_ESTQTERCFIN2@LINK_C5(E.SEQPRODUTO, E.NROEMPRESA)) ESTOQUE_DISPONIVEL,
       X.PRECOVALIDNORMAL                               NORMAL_PRICE,
       ROUND(X.PRECOVALIDNORMAL * 1.08,2)               NORMAL_PRICE_MKP,
       X.PRECOVALIDPROMOC                               PROMOC_PRICE,
       NVL(NULLIF(X.PRECOVALIDPROMOC,0), X.PRECOVALIDNORMAL)      ACTUAL_PRICE,
       ...
       E.DTAULTVENDA                                    ,
       E.DTAULTENTRADA                                DTAULTENTRADA,
       ROUND(E.MEDVDIAGERAL,2)                          MEDIA_VENDA,
       MARCA                                            ,
       CATEGORIAN1                                      CATEGORIA  ,
       fUrl@LINK_C5(X.SEQPRODUTO)                               URL,
       CASE WHEN R.SEQPRODUTO IS NOT NULL THEN 'S' ELSE 'N' END EX_PROX_VALID

  FROM MRL_PRODEMPSEG@LINK_C5 X INNER JOIN DIM_PRODUTO P ON P.SEQPRODUTO = X.SEQPRODUTO
                        INNER JOIN MRL_PRODUTOEMPRESA@LINK_C5 E ON E.NROEMPRESA = X.NROEMPRESA AND E.SEQPRODUTO = NVL(P.SEQPRODUTOBASE, X.SEQPRODUTO)
                        INNER JOIN DIM_EMPRESA M ON M.NROEMPRESA = X.NROEMPRESA AND X.NROSEGMENTO = M.NROSEGMENTOPRINC AND E.NROEMPRESA < 100
                         LEFT JOIN NAGV_MENORPROMOCATIVA@LINK_C5 PROMOC ON PROMOC.SKU = X.SEQPRODUTO AND PROMOC.STOREREFERENCE = X.NROEMPRESA
                         LEFT JOIN NAGV_BASE_PROMOC_PDV@LINK_C5 MN ON MN.SEQPRODUTO = X.SEQPRODUTO AND MN.NROEMPRESA = X.NROEMPRESA AND MN.CODPARCEIRO = 700
                        INNER JOIN DIM_CATEGORIA T ON  T.SEQFAMILIA = P.SEQFAMILIA
                         LEFT JOIN MRL_PROMOCESPECIALHIST@LINK_C5 R ON R.SEQPRODUTO = X.SEQPRODUTO AND R.NROEMPRESA = X.NROEMPRESA AND TRUNC(SYSDATE) BETWEEN R.DTAINICIO AND R.DTAFIM
...
SELECT /*+ ORDERED */
  ...
  FROM MRL_PRODEMPSEG X INNER JOIN MAP_PRODUTO P ON P.SEQPRODUTO = X.SEQPRODUTO
                        ...
                        INNER JOIN MAX_EMPRESA M ON M.NROEMPRESA = X.NROEMPRESA AND X.NROSEGMENTO = M.NROSEGMENTOPRINC AND E.NROEMPRESA < 100
