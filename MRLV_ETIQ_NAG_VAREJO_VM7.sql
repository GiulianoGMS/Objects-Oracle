CREATE OR REPLACE VIEW MRLV_ETIQ_NAGUMO_VAREJO_VM7 AS
SELECT /*+OPTIMIZER_FEATURES_ENABLE('11.2.0.4')*/
        -- Novo modelo criado por Giuliano | 28/05/2025
        -- Atualizada com Regras de Incentivo em 21/10/2025
...
--======= Quando existir Meu Nagumo e Oferta Normal
          ...
          WHEN (NVL(J.PRECOVALIDPROMOC,0) > 0) AND (NVL(J.PRECOVALIDPROMOC,0))  < TRUNC((A.PRECOVALIDNORMAL * A.PADRAOEMBVENDA),2) AND J.QTDEMBALAGEM = 1 AND RPM.PRECOPROMOCAO IS NOT NULL  -- Se tem MN
               AND RPM.PRECOPROMOCAO < J.PRECOVALIDPROMOC THEN -- se MN é menor que oferta
       -- ALTERADO POR GIULIANO | TRATAMENTO PRECO MEU NAGUMO
       CASE WHEN RPM.PRECOPROMOCAO < NVL(J.PRECOVALIDPROMOC,0) AND RPM.PRECOPROMOCAO > 0-- SE EXISTIR MEU NAGUMO, FORMA OFERTA + MEU NAGUMO E FOR MENOR QUE OFERTA NORMAL
       ...
       THEN                                   CHR(13) || CHR(10) ||
        ...
        '^FO590,128^A0N,15,15^FDNesta Embalagem '||
                                CASE WHEN J.MULTEQPEMBALAGEM = 'LI' THEN 'LITRO' WHEN  J.MULTEQPEMBALAGEM IN ('GR','MG') THEN '100g' WHEN J.MULTEQPEMBALAGEM = 'ML' THEN '100ml' ELSE J.MULTEQPEMBALAGEM END ||' R$ ' ||
                                DECODE(SIGN(J.PRECOGERPROMOC),+1,
                                TRANSLATE(TO_CHAR(ROUND((J.PRECOGERPROMOC/(J.MULTEQPEMB*1000)) * CASE WHEN J.MULTEQPEMBALAGEM IN ('GR','ML', 'MG') THEN 100000  END ,2),'FM9990.00'), '.', ','),
                                TRANSLATE(TO_CHAR(ROUND((J.PRECOGERNORMAL/(J.MULTEQPEMB*1000)) * CASE WHEN J.MULTEQPEMBALAGEM IN ('GR','ML', 'MG') THEN 100000  END ,2),'FM9990.00'), '.', ','))  END )||'^FS' || CHR(13) || CHR(10) ||
        ...
        '^FO580,167^A0N,58,58^FD'||LPAD(TRUNC(RPM.PRECOPROMOCAO), 4, ' ' ) || ','||
                                   LPAD((RPM.PRECOPROMOCAO -
                                   TRUNC(RPM.PRECOPROMOCAO)) * 100, 2, 0)||'^FR^FS'|| CHR(13) || CHR(10) ||
        (CASE WHEN J.MULTEQPEMB IS NOT NULL  OR J.MULTEQPEMBALAGEM IS NOT NULL THEN
        '^FO590,221^A0N,15,15^FDNesta Embalagem '||
                                CASE WHEN J.MULTEQPEMBALAGEM = 'LI' THEN 'LITRO' WHEN  J.MULTEQPEMBALAGEM IN ('GR','MG') THEN '100g' WHEN J.MULTEQPEMBALAGEM = 'ML' THEN '100ml' ELSE J.MULTEQPEMBALAGEM END ||' R$ ' ||
                                DECODE(SIGN(RPM.PRECOPROMOCAO),+1,
                                TRANSLATE(TO_CHAR(ROUND((RPM.PRECOPROMOCAO/(J.MULTEQPEMB*1000)) * CASE WHEN J.MULTEQPEMBALAGEM IN ('GR','ML', 'MG') THEN 100000  END ,2),'FM9990.00'), '.', ','),
                                TRANSLATE(TO_CHAR(ROUND((RPM.PRECOPROMOCAO/(J.MULTEQPEMB*1000)) * CASE WHEN J.MULTEQPEMBALAGEM IN ('GR','ML', 'MG') THEN 100000  END ,2),'FM9990.00'), '.', ',')) END)||'^FS'|| CHR(13) || CHR(10) ||
        '^FO490,155^GB300,86,51^FR^FS'     || CHR(13) || CHR(10) ||
        '^FO490,60^GB300,85,50^FR^FS'

        END

--======= Preco para Regra de Incentivo + Oferta
      -- Tipo RO

        WHEN (NVL(J.PRECOVALIDPROMOC,0) > 0) AND (NVL(J.PRECOVALIDPROMOC,0))  < TRUNC((A.PRECOVALIDNORMAL * A.PADRAOEMBVENDA),2) AND J.QTDEMBALAGEM = 1
         AND NVL(RI.PRECO_FINAL,0) > 0 AND RI.PRECO_FINAL < NVL(J.PRECOVALIDPROMOC,0) AND 1=2
        ...
        '^FO50,20^A0N,40,40^FD'||SUBSTR(A.DESCCOMPLETA,0,40) ||' '||CASE WHEN J.QTDEMBALAGEM > 1 THEN J.EMBALAGEM ELSE NULL END||'^FS' || CHR(13) || CHR(10) || -- DESC
        '^FO30,251^A0N,20,20^FD'||TO_CHAR(SYSDATE, 'DD/MM/YY HH24:MI')||'   Produto: '||A.SEQPRODUTO||'   Val. Prom.: '||
                                TO_CHAR(RI.DTAINICIO, 'DD/MM/YY')||' a ' ||
                                TO_CHAR(RI.DTAFIM, 'DD/MM/YY')|| ' RO  LJ: ' || G.NOMEREDUZIDO||'^FS' || CHR(13) || CHR(10) ||
        '^FO275,90^A0N,35,35^FDOFERTA^FS'  || CHR(13) || CHR(10) ||
        '^FO490,100^A0N,30,30^FD  R$  ^FS' || CHR(13) || CHR(10) ||
        '^FO495,76^A0N,20,20^FD  POR  ^FS' || CHR(13) || CHR(10) ||
        '^FO580,73^A0N,58,58^FD'||LPAD(TRUNC(J.PRECOVALIDPROMOC), 4, ' ' ) || ',' || LPAD((J.PRECOVALIDPROMOC - TRUNC(J.PRECOVALIDPROMOC)) * 100, 2, 0)||'^FS' || CHR(13) || CHR(10) || -- PROMOC
        ...
        '^FO490,195^A0N,30,30^FD  R$  ^FS' || CHR(13) || CHR(10) ||
        '^FO495,175^A0N,20,20^FD  POR  ^FS'|| CHR(13) || CHR(10) ||
        '^FO275,165^GFA,704,704,11,,::::V07C,U0FFE,S01IFE,R01JFE,Q01KFE,P07MF,O0OF,M01PF,L01MFC3FF,K03NF01FF,J07NFE00FF8,I07OFE007F8,00QFC387F8,1RFC3C7F8,3RFC3C7FC,3RFC3C3FC,3RFE3C0FC,3MF3JFE1E0FC,3FFC3FFA1JFE1F1FC,3FC0FF80DJFE1IFE,1IFE1801KF7IFE,1FFE4081PFE,1FCI03QFE,1FE003RFE,0FE0UF,0FEVF,0XF,::07WF8,:::03WF8,03WFC,03UFBFC,03UF03C,03TFE03C,01UF1FC,01VFBE,01UFC7E,01WFC,01VF,00UF,00SFE,00RFE,00QFC,00PFC,007NF8,007MF,007KFC,007JF8,007IF8,007FF,003E,,:::^FS'|| -- Imagem
        '^FO370,180^A0N,20,20^FDNO CARTAO^FS'||
        '^FO380,200^A0N,20,20^FDNAGUMO^FS'||
        '^FO580,167^A0N,58,58^FD'||LPAD(TRUNC(RI.PRECO_FINAL), 4, ' ' ) || ','||
                                   LPAD((RI.PRECO_FINAL -
                                   TRUNC(RI.PRECO_FINAL)) * 100, 2, 0)||'^FR^FS'|| CHR(13) || CHR(10) ||
        (CASE WHEN J.MULTEQPEMB IS NOT NULL  OR J.MULTEQPEMBALAGEM IS NOT NULL THEN
        '^FO590,221^A0N,15,15^FDNesta Embalagem '||
                                CASE WHEN J.MULTEQPEMBALAGEM = 'LI' THEN 'LITRO' WHEN  J.MULTEQPEMBALAGEM IN ('GR','MG') THEN '100g' WHEN J.MULTEQPEMBALAGEM = 'ML' THEN '100ml' ELSE J.MULTEQPEMBALAGEM END ||' R$ ' ||
                                DECODE(SIGN(RI.PRECO_FINAL),+1,
                                TRANSLATE(TO_CHAR(ROUND((RI.PRECO_FINAL/(J.MULTEQPEMB*1000)) * CASE WHEN J.MULTEQPEMBALAGEM IN ('GR','ML', 'MG') THEN 100000  END ,2),'FM9990.00'), '.', ','),
                                TRANSLATE(TO_CHAR(ROUND((RI.PRECO_FINAL/(J.MULTEQPEMB*1000)) * CASE WHEN J.MULTEQPEMBALAGEM IN ('GR','ML', 'MG') THEN 100000  END ,2),'FM9990.00'), '.', ',')) END)||'^FS'|| CHR(13) || CHR(10) ||
        '^FO490,155^GB300,86,51^FR^FS'     || CHR(13) || CHR(10) ||
        '^FO490,60^GB300,85,50^FR^FS'
...
--======= Em promocao e a Promocao e MENOR que o PRECO NORMAL, e nao existe Preco A Partir De em Combos Promocionais
       ...
       -- OFERTA NORMAL
        ...
        '^FO590,215^A0N,15,15^FDNesta Embalagem '||
                                CASE WHEN J.MULTEQPEMBALAGEM = 'LI' THEN 'LT' WHEN  J.MULTEQPEMBALAGEM IN ('GR','MG') THEN '100g' WHEN J.MULTEQPEMBALAGEM = 'ML' THEN '100ml' ELSE J.MULTEQPEMBALAGEM END ||' R$ ' ||
                                DECODE(SIGN(J.PRECOGERPROMOC),+1,
                                TRANSLATE(TO_CHAR(ROUND((J.PRECOGERPROMOC/(J.MULTEQPEMB*1000)) * CASE WHEN J.MULTEQPEMBALAGEM IN ('GR','ML', 'MG') THEN 100000  END ,2),'FM9990.00'), '.', ','),
                                TRANSLATE(TO_CHAR(ROUND((J.PRECOGERNORMAL/(J.MULTEQPEMB*1000)) * CASE WHEN J.MULTEQPEMBALAGEM IN ('GR','ML', 'MG') THEN 100000  END ,2),'FM9990.00'), '.', ','))END )||'^FS' || CHR(13) || CHR(10) ||
        '^FO690,85^A0N,090,40^FD  ^FS'    || CHR(13) || CHR(10) ||
...
--======== INICIO OFERTAS EXCLUSIVAS - APENAS MEU NAGUMO SEM OFERTA C5 - 02/09/2024 - Giuliano ========--
         ...
         WHEN NVL(J.PRECOVALIDPROMOC,0) = 0 AND RPM.PRECOPROMOCAO < TRUNC((A.PRECOVALIDNORMAL * A.PADRAOEMBVENDA),2) AND RPM.PRECOPROMOCAO > 0 AND J.QTDEMBALAGEM = 1 AND RPM.PRECOPROMOCAO IS NOT NULL -- Se tem MN
         THEN                                   CHR(13) || CHR(10) ||
        ...
        '^FO50,251^A0N,20,20^FD'||TO_CHAR(SYSDATE, 'DD/MM/YY HH24:MI')||'   Produto: '||A.SEQPRODUTO||'   Val. Prom.: '||
                                  TO_CHAR(RPM.DTAINICIO, 'DD/MM/YY')||' a ' ||
                                  TO_CHAR(RPM.DTAFIM,    'DD/MM/YY')|| ' M  LJ: ' || G.NOMEREDUZIDO||'^FS' || CHR(13) || CHR(10) ||
        ...
        (CASE WHEN J.PRECOVALIDPROMOC > 99.99
        THEN
        '^FO515,80^A0N,150,110^FD'||LPAD(TRUNC(RPM.PRECOPROMOCAO), 4, ' ' ) || ',' || LPAD((RPM.PRECOPROMOCAO - TRUNC(RPM.PRECOPROMOCAO)) * 100, 2, '0')|| '^FS'
        ELSE
        '^FO475,80^A0N,150,110^FD'||LPAD(TRUNC(RPM.PRECOPROMOCAO), 4, ' ' ) || ',' || LPAD((RPM.PRECOPROMOCAO - TRUNC(RPM.PRECOPROMOCAO)) * 100, 2, '0')|| '^FS' END)|| CHR(13) || CHR(10) || -- PROMOC
        (CASE WHEN J.MULTEQPEMB IS NOT NULL  OR J.MULTEQPEMBALAGEM IS NOT NULL THEN
        '^FO590,215^A0N,15,15^FDNesta Embalagem '||
                                CASE WHEN J.MULTEQPEMBALAGEM = 'LI' THEN 'LITRO' WHEN  J.MULTEQPEMBALAGEM IN ('GR','MG') THEN '100g' WHEN J.MULTEQPEMBALAGEM = 'ML' THEN '100ml' ELSE J.MULTEQPEMBALAGEM END ||' R$ ' ||
                                DECODE(SIGN(J.PRECOGERPROMOC),+1,
...
--======== INICIO OFERTAS EXCLUSIVAS - APENAS CARTAO NAGUMO E SEM OFERTA C5 - 21/10/2025 - Giuliano ========--

         WHEN NVL(J.PRECOVALIDPROMOC,0) = 0 AND RI.PRECO_FINAL < TRUNC((A.PRECOVALIDNORMAL * A.PADRAOEMBVENDA),2) AND RI.PRECO_FINAL > 0 AND J.QTDEMBALAGEM = 1 AND RI.PRECO_FINAL IS NOT NULL -- Se tem MN
         THEN                                   CHR(13) || CHR(10) ||
        '^FO50,20^A0N,40,40^FD'||SUBSTR(A.DESCCOMPLETA,0,37) ||' '||CASE WHEN J.QTDEMBALAGEM > 1 THEN J.EMBALAGEM ELSE NULL END||'^FS' || CHR(13) || CHR(10) || -- DESC
        '^FO50,251^A0N,20,20^FD'||TO_CHAR(SYSDATE, 'DD/MM/YY HH24:MI')||'   Produto: '||A.SEQPRODUTO||'   Val. Prom.: '||
                                  TO_CHAR(RI.DTAINICIO, 'DD/MM/YY')||' a ' ||
                                  TO_CHAR(RI.DTAFIM,    'DD/MM/YY')|| ' R  LJ: ' || G.NOMEREDUZIDO||'^FS' || CHR(13) || CHR(10) ||
        ...
        (CASE WHEN J.PRECOVALIDPROMOC > 99.99
        THEN
        '^FO515,80^A0N,150,110^FD'||LPAD(TRUNC(RI.PRECO_FINAL), 4, ' ' ) || ',' || LPAD((RI.PRECO_FINAL - TRUNC(RI.PRECO_FINAL)) * 100, 2, '0')|| '^FS'
        ELSE
        '^FO475,80^A0N,150,110^FD'||LPAD(TRUNC(RI.PRECO_FINAL), 4, ' ' ) || ',' || LPAD((RI.PRECO_FINAL - TRUNC(RI.PRECO_FINAL)) * 100, 2, '0')|| '^FS' END)|| CHR(13) || CHR(10) || -- PROMOC
        (CASE WHEN J.MULTEQPEMB IS NOT NULL  OR J.MULTEQPEMBALAGEM IS NOT NULL THEN
        '^FO590,215^A0N,15,15^FDNesta Embalagem '||
                                CASE WHEN J.MULTEQPEMBALAGEM = 'LI' THEN 'LITRO' WHEN  J.MULTEQPEMBALAGEM IN ('GR','MG') THEN '100g' WHEN J.MULTEQPEMBALAGEM = 'ML' THEN '100ml' ELSE J.MULTEQPEMBALAGEM END ||' R$ ' ||
                                DECODE(SIGN(J.PRECOGERPROMOC),+1,
        ...
        '^FO50,95^A0N,25,25^FDR$^FS'|| CHR(13) || CHR(10) ||
        '^FO51,125^A0N,20,20^FD^FS'|| CHR(13) || CHR(10) ||
        '^FO270,75,0^'||(SELECT LT_IMG FROM CONSINCO.NAGT_LT_IMG3 WHERE TIPO = 5) || CHR(13) || CHR(10) || -- Imagem MN
        -- TRATATIVA LENGTH - TAMANHO PRECO NORMAL
        (CASE WHEN J.PRECOVALIDNORMAL > 99.99
        THEN
        '^FO40,75^A0N,62,62^FD'||LPAD(TRUNC(J.PRECOVALIDNORMAL), 4, ' ' ) || ',' || LPAD((J.PRECOVALIDNORMAL - TRUNC(J.PRECOVALIDNORMAL)) * 100, 2, '0')|| '^FS'
...
--======= Combos Promocionais - A Partir de por CAIXA
       ...
       -- Tipo B
        ...
        '^FO590,215^A0N,15,15^FDNesta Embalagem '||
                                ...
                                TRANSLATE(TO_CHAR(ROUND((PPDV.PRECOPROMOCAO/(J.MULTEQPEMB*1000)) * CASE WHEN J.MULTEQPEMBALAGEM IN ('GR','ML', 'MG') THEN 100000  END ,2),'FM9990.00'), '.', ','),
                                TRANSLATE(TO_CHAR(ROUND((PPDV.PRECOPROMOCAO/(J.MULTEQPEMB*1000)) * CASE WHEN J.MULTEQPEMBALAGEM IN ('GR','ML', 'MG') THEN 100000  END ,2),'FM9990.00'), '.', ','))END )||'^FS' || CHR(13) || CHR(10) ||
        '^FO690,85^A0N,090,40^FD  ^FS'    || CHR(13) || CHR(10) ||
        '^FO50,95^A0N,25,25^FDR$^FS'      || CHR(13) || CHR(10) ||
        '^FO51,125^A0N,20,20^FDDE^FS'     || CHR(13) || CHR(10) ||
...
--======== Combos Promocionais - A Partir de por UNIDADE
       ...
       THEN
        ...
        '^FO590,128^A0N,15,15^FDNesta Embalagem '||
                                ...
                                TRANSLATE(TO_CHAR(ROUND(((A.PRECOEMBPADRAO * J.QTDEMBALAGEM)/(J.MULTEQPEMB*1000)) * CASE WHEN J.MULTEQPEMBALAGEM IN ('GR','ML', 'MG') THEN 100000  END ,2),'FM9990.00'), '.', ','),
                                TRANSLATE(TO_CHAR(ROUND(((A.PRECOEMBPADRAO * J.QTDEMBALAGEM)/(J.MULTEQPEMB*1000)) * CASE WHEN J.MULTEQPEMBALAGEM IN ('GR','ML', 'MG') THEN 100000  END ,2),'FM9990.00'), '.', ','))  END )||'^FS' || CHR(13) || CHR(10) ||
        '^FO690,85^A0N,090,40^FD  ^FS'     || CHR(13) || CHR(10) ||
        '^FO260,60^GB230,85,1^FS'          || CHR(13) || CHR(10) ||
        '^FO260,155^GB530,85,1^FS'         || CHR(13) || CHR(10) ||
        '^FO490,195^A0N,30,30^FD  R$  ^FS' || CHR(13) || CHR(10) ||
        ...
        '^FO590,221^A0N,15,15^FDNesta Embalagem '||
                                ...
                                TRANSLATE(TO_CHAR(ROUND(((PPDV.PRECOPROMOCAO * J.QTDEMBALAGEM)/(J.MULTEQPEMB*1000)) * CASE WHEN J.MULTEQPEMBALAGEM IN ('GR','ML', 'MG') THEN 100000  END ,2),'FM9990.00'), '.', ','),
                                TRANSLATE(TO_CHAR(ROUND(((PPDV.PRECOPROMOCAO * J.QTDEMBALAGEM)/(J.MULTEQPEMB*1000)) * CASE WHEN J.MULTEQPEMBALAGEM IN ('GR','ML', 'MG') THEN 100000  END ,2),'FM9990.00'), '.', ',')) END)||'^FS'|| CHR(13) || CHR(10) ||
        '^FO490,155^GB300,86,51^FR^FS'     || CHR(13) || CHR(10) ||
        -- QrCode
...
--======== Gerenciador de Precos - A Partir de por CAIXA
        ...
        '^FO590,215^A0N,15,15^FDNesta Embalagem '||
                                ...
                                TRANSLATE(TO_CHAR(ROUND((P.PRECOVALIDNORMAL/(J.MULTEQPEMB*1000)) * CASE WHEN J.MULTEQPEMBALAGEM IN ('GR','ML', 'MG') THEN 100000  END ,2),'FM9990.00'), '.', ','),
                                TRANSLATE(TO_CHAR(ROUND((P.PRECOVALIDNORMAL/(J.MULTEQPEMB*1000)) * CASE WHEN J.MULTEQPEMBALAGEM IN ('GR','ML', 'MG') THEN 100000  END ,2),'FM9990.00'), '.', ','))END )||'^FS' || CHR(13) || CHR(10) ||
        '^FO690,85^A0N,090,40^FD  ^FS'    || CHR(13) || CHR(10) ||
        '^FO50,95^A0N,25,25^FDR$^FS'      || CHR(13) || CHR(10) ||
        '^FO51,125^A0N,20,20^FDDE^FS'     || CHR(13) || CHR(10) ||
        '^FO70,92^A0N,60,60^FD'||LPAD(TRUNC(A.PRECOEMBPADRAO * J.QTDEMBALAGEM), 4, ' ' ) || ',' || LPAD(((A.PRECOEMBPADRAO * J.QTDEMBALAGEM) - TRUNC(A.PRECOEMBPADRAO * J.QTDEMBALAGEM)) * 100, 2, '0')|| '^FS' || CHR(13) || CHR(10) || -- PRECO NORMAL
...
--======== Gerenciador de Precos - A Partir de por UNIDADE
       ...
       THEN
        ...
        '^FO590,221^A0N,15,15^FDNesta Embalagem '||
                                ...
                                TRANSLATE(TO_CHAR(ROUND(((ATAC.PRECOATAC * J.QTDEMBALAGEM)/(J.MULTEQPEMB*1000)) * CASE WHEN J.MULTEQPEMBALAGEM IN ('GR','ML', 'MG') THEN 100000  END ,2),'FM9990.00'), '.', ','),
                                TRANSLATE(TO_CHAR(ROUND(((ATAC.PRECOATAC * J.QTDEMBALAGEM)/(J.MULTEQPEMB*1000)) * CASE WHEN J.MULTEQPEMBALAGEM IN ('GR','ML', 'MG') THEN 100000  END ,2),'FM9990.00'), '.', ',')) END)||'^FS'|| CHR(13) || CHR(10) ||
        '^FO490,155^GB300,86,51^FR^FS'     || CHR(13) || CHR(10) ||
        -- QrCode
        '^LH0,0^FO50,70^BQN,2,4 ^FDMA,'||CASE WHEN J.NROSEGMENTO NOT IN (8,4) THEN 'qrco.de/bg6UzU' ELSE 'qrco.de/bg0jxt' END||'^FS'     || CHR(13) || CHR(10) ||
        -- Imagem | Logo
...
CHR(13) || CHR(10) || '^FO550,250^A0N,15,16^FD' ||/* '*PRECO PAGO POR ' || J.MULTEQPEMBALAGEM ||' R$   '*/'Nesta Embalagem ' || CASE WHEN J.MULTEQPEMBALAGEM = 'LI' THEN 'LT'
      ...
      || DECODE(SIGN(J.PRECOGERPROMOC/*J.PRECOGERNORMAL*/),+1,
       TRANSLATE(TO_CHAR(ROUND((J.PRECOGERPROMOC/(J.MULTEQPEMB*1000)) * CASE WHEN J.MULTEQPEMBALAGEM IN ('GR','ML', 'MG') THEN 100000  END ,2),'FM9990.00'), '.', ','),
       TRANSLATE(TO_CHAR(ROUND((J.PRECOGERNORMAL/(J.MULTEQPEMB*1000)) * CASE WHEN J.MULTEQPEMBALAGEM IN ('GR','ML', 'MG') THEN 100000  END ,2),'FM9990.00'), '.', ','))
|| CHR(13) || CHR(10) || ''
END)||'^FR^FS'
...
FROM CONSINCO.MRLX_BASEETIQUETAPROD A INNER JOIN CONSINCO.MRLV_BASEETIQUETAPROD_NAGV2 J ON J.NROEMPRESA  = A.NROEMPRESA AND J.SEQPRODUTO = A.SEQPRODUTO AND J.NROSEGMENTO = A.NROSEGMENTO AND J.QTDEMBALAGEM = A.QTDEMBCODACESSO
                                      ...
                                      -- Joins de atacado
                                       LEFT JOIN NAGV_BASE_PROMOC_PDV PPDV ON PPDV.SEQPRODUTO = A.SEQPRODUTO AND PPDV.NROEMPRESA = A.NROEMPRESA AND PPDV.CODPARCEIRO IS NULL
                                       LEFT JOIN NAGV_BASE_PROMOC_PRECO2 P ON P.NROEMPRESA  = A.NROEMPRESA AND P.SEQPRODUTO = A.SEQPRODUTO AND P.NROSEGMENTO = A.NROSEGMENTO AND P.QTDEMBALAGEM = A.QTDEMBCODACESSO AND 1=2
                                                                 ...
                                                                 (SELECT MIN(EMPS.QTDEMBALAGEM)
                                                                       ...
                                                                       WHERE EMPS.SEQPRODUTO = ATAC.SEQPRODUTO
                                                                         ...
                                                                         AND EMPS.QTDEMBALAGEM > 1)
                                                                                                           ...
                                                                                                           AND ATAC.SEQPRODUTO  = A.SEQPRODUTO AND 1=2
                                       ...
                                       -- Novo join -- Precos Meu Nagumo - Identificados
                                       LEFT JOIN NAGV_BASE_PROMOC_PDV RPM ON RPM.NROEMPRESA = A.NROEMPRESA AND RPM.SEQPRODUTO = A.SEQPRODUTO AND RPM.CODPARCEIRO = 700
                                       -- Join das Regras de Incentivo
                                       LEFT JOIN NAGV_PROMOC_ATIVAS_REGRAPDV RI ON RI.SEQPRODUTO = A.SEQPRODUTO AND RI.NROEMPRESA = A.NROEMPRESA
