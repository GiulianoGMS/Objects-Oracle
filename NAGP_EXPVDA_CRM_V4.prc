CREATE OR REPLACE PROCEDURE NAGP_EXPVDA_CRM_V4 (vsDtaInicial DATE, vsDtaFinal DATE) IS
...
BEGIN
    ...
    v_file := UTL_FILE.fopen('PLUSOFT', 'Ext_CRM_Vendas_'||v_Periodo||'.csv', 'w', 32767); 

    -- Pega o nome das colunas para inserir no cabecalho pq tenho preguica
    SELECT LISTAGG(COLUMN_NAME,';') WITHIN GROUP (ORDER BY COLUMN_ID)
      INTO v_Cabecalho
      FROM ALL_TAB_COLUMNS A
     WHERE A.table_name = 'NAGV_CRM_VENDAS_EXT_V2'
