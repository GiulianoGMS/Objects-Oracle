CREATE OR REPLACE VIEW MRLV_ETIQ_NAGUMO_VAREJO_VM6 AS
...
--======= Preco para Regra de Incentivo + Oferta
      -- Tipo RO
        ...
        WHEN (NVL(J.PRECOVALIDPROMOC,0) > 0) AND (NVL(J.PRECOVALIDPROMOC,0))  < TRUNC((A.PRECOVALIDNORMAL * A.PADRAOEMBVENDA),2) AND J.QTDEMBALAGEM = 1
         AND NVL(RI.PRECO_FINAL,0) > 0 AND RI.PRECO_FINAL < NVL(J.PRECOVALIDPROMOC,0)
...
--======== INICIO OFERTAS EXCLUSIVAS - APENAS MEU NAGUMO SEM OFERTA C5 - 02/09/2024 - Giuliano ========--
        ...
        '^FO50,251^A0N,20,20^FD'||TO_CHAR(SYSDATE, 'DD/MM/YY HH24:MI')||'   Produto: '||A.SEQPRODUTO||'   Val. Prom.: '||
                                  ...
                                  TO_CHAR(RPM.DTFIM,    'DD/MM/YY')|| ' M  LJ: ' || G.NOMEREDUZIDO||'^FS' || CHR(13) || CHR(10) ||
